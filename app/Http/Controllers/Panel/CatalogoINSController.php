...
class CatalogoINSController extends Controller
{
    private $general;
    ...
    public function index(Request $request)
    {
        ...
        $url = env("API_DIR")."getOcupacionINS";
        $tipo = "POST";
        ...
        if(isset($request['buscar']) && trim($request['buscar']) != "")
        {
            ...
            $parametros .= ',"columna" : "categoria_perfil_ocupacional"';
        ...
        $conjuntoResultados = [
            ...
            'total'=>$catalogoOcupaciones['total_ocupaciones'],
            'total_paginas'=>$catalogoOcupaciones['total_paginas'],
            'cantidad'=>$cantidad,
            'paginaActual'=>$paginaActual,
        ];

        return view('panel.configuracion.catalogoINS.catalogoINS_index',$conjuntoResultados);
    }

    public function create()
    {
        session()->now('dropZone','1');
        return view('panel.configuracion.catalogoINS.catalogoINS_create');
    }
        ...
        if($request->hasFile("archivo_excel")) {
            ...
            foreach ($datos as $fila)
            {
                if(trim($fila[0])!="")
                {
                    ...
                    $rowArray['categoria_perfil_ocupacional']= trim($fila[1]);
                    $array_catalogo[]=$rowArray;
                }
            }

            //variables a usar en el api
            $url = env("API_DIR")."insertarOcupacionINS";
            $conjuntoParametros = [
    ...
    public function descargarPlantilla()
    {
        ...
        $path =public_path() . '/files/panel/Plantilla_Ocupaciones_INS.xlsx';
    ...
    public function descargar_excel(Request $request){
        ...
        $url = env("API_DIR")."exportarExcelOcupacionesINS";
        ...
        $conjuntoParametros = [
            ...
            'columna' => isset($request['columna']) ? $request['columna'] : '', // consultar si se tiene que colocar por default -> categoria_perfil_ocupacional

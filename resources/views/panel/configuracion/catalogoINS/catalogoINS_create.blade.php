...
@section('page-content')
    <form id="guardarExcel" action="{{ route('configuracionCatalogoINS.store') }}" method="post" enctype="multipart/form-data">
        @csrf
        <h5 class="card-title text-110 text-success pt-1">
            Cargando catálogo de ocupaciones INS
        </h5>
        <div class="mt-4">
...
@push('scripts')
    ...
    <script type="module">
        ...
        $('#guardarExcel').submit(function(e) {
            ...
            $.ajax({
                type:'POST',
                url: "{{ route('configuracionCatalogoINS.store') }}",
                data: formData,
                contentType: false,
                processData: false,
                success: (response) => {

...
@section('page-content')
    ...
    <input type="hidden" id="paginaActual" value="{{$paginaActual??"" }}"/>
    <input type="hidden" id="buscar" value="{{ $buscar??"" }}"/>
    <input type="hidden" id="orden" value="{{ $orden??"" }}"/>
    <input type="hidden" id="tipo_orden" value="{{ $tipo_orden??"" }}"/>
    <input type="hidden" id="url_generarPlanilla_edit_blade" value="{{ route('generarPlanilla.show', [Crypt::encrypt($idPlanilla)]) }}"/>
        ...
        <div class="col-lg-2 px-0">
            <div class="row">
                <!--<div class="form-group col-12 col-lg mb-0 mt-1 mt-md-0">
                    <label class="text-primary-d1 text-95">Salario Neto</label>
                    <input readonly="" id="id-disable-me" type="text" class="form-control" value="{{number_format($planillaTotalNeto,2)}}">
                </div>-->
                ...
                <div class="form-group col-12 col-lg mb-0 mt-1 mt-md-0">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control" value="{{ number_format($planillaTotalDevengado, 2, ".", " ") }}" readonly/>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-lg-6 my-2 my-lg-0 px-0 px-lg-2 text-nowrap">
            <div class="d-block d-lg-inline mt-2 mt-lg-0">
                 <span class="mt-35 mr-1" id="headingOne">
                    <button id="btn2" class="btn px-2 btn-light-purple font-bolder letter-spacing text-95" type="button" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                       <!-- <i class="fa fa-gears text-110 text-purple-d2 mr-1"></i>-->
                        Extras
                    </button>
                </span>

                <span class="d-none mr-1" id="headingTwo">
                    <button id="guardarFeriados" class="btn px-2 btn-light-success font-bolder letter-spacing text-95  collapsed" type="button" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
                       <!-- <i class="fa fa-gears text-110 text-purple-d2 mr-1"></i>-->
                        Tabla principal
                    </button>
                </span>
            ...
            <span class="mt-35">
                <a href="#" data-toggle="modal" data-target="#chequeoPlanilla" class="mr-1 btn px-2 btn-light-orange font-bolder letter-spacing text-95">
    ...
    <!--control de chequeo-->
    <div class="modal fade" id="chequeoPlanilla" tabindex="-1" role="dialog" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title text-blue-d2">
                        Control de chequeo al generar la planilla
                    </h5>
                ...
                <div class="modal-body ace-scrollbar">
                    A continuación antes de guardar la planilla de forma definitiva confirme que todos los puntos de control que se enuncian estén aplicados.
                    <br><br>
                    <!--<div class="card-body bgc-white border-1 border-t-0 brc-success-m3">
                        <div class="jqtree tree-dotted" id="id-jqtree-files-planilla"></div>
                    </div>-->

                    <div class="ml-45">
                        <div class="mt-1 mb-1">
                            <input type="checkbox" name="chkVerificar" class="bgc-primary" value="1"> Se registraron horas extra
                        </div>
                        <div class="mt-1 mb-1">
                            <input type="checkbox" name="chkVerificar" class="bgc-primary" value="1"> Se registraron feriados
                        </div>
                        <div class="mt-1 mb-1">
                            <input type="checkbox" name="chkVerificar" class="bgc-primary" value="1"> Se registraron vacaciones
                        </div>
                        <div class="mt-1 mb-1">
                            <input type="checkbox" name="chkVerificar" class="bgc-primary" value="1"> Se registraron permisos
                        </div>
                        <div class="mt-1 mb-1">
                            <input type="checkbox" name="chkVerificar" class="bgc-primary" value="1"> Se registraron incapacidades y licencias
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="continuarArchivoTxt" class="btn btn-primary" data-dismiss="modal">
                        Continuar
                    </button>
                </div>
    ...
    <!--Descargar txt-->
    <form method="GET" id="frm-descargarTxt" action="{{ route('descargarTxtPlanilla.download',[Crypt::encrypt($idPlanilla)]) }}">
        @csrf
            ...
            <div class="modal-dialog modal-dialog-message modal-dialog-scrollable modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button id="guardar-txt" type="button" class="btn btn-primary">
                            Continuar
                        </button>
                    </div>
    ...
    <!--Descargar excel-->
    <form method="GET" id="frm-descargarExcel" action="{{ route('descargarExcelPlanilla.download',[Crypt::encrypt($idPlanilla)]) }}">
        @csrf
            ...
            <div class="modal-dialog modal-dialog-message modal-dialog-scrollable modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button id="guardar-excel" type="button" class="btn btn-primary">
                            Continuar
                        </button>
                    </div>
    ...
    <form method="POST" action="{{ route('registrarPlanilla',[$idPlanilla]) }}">
        @csrf
        @method('PUT')
        ...
        <div class="modal fade" id="cierrePlanilla" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog modal-dialog-message modal-dialog-scrollable modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title text-blue-d2">
                            ¿Está seguro de Realizar el cierre de planilla?
                        </h5>
                    ...
                    <div class="modal-body ace-scrollbar">
                        Esta acción realiza un bloqueo definitivo sobre los cálculos realizados y no podrá efectuar ningún cambio o modificación, posterior al cierre de la planilla.
                    </div>
...
@push('scripts')
    ...
    <script type="module">
        ...
        $("#continuarArchivoTxt").on('click', function (evt)
        {
            var totalCheckboxs = $("input[name=chkVerificar]").length;
            var totalMarcados  = $("input[name=chkVerificar]:checked").length;

            if(totalCheckboxs == totalMarcados){
                $('#chequeoPlanilla').modal('hide');
                $('#descargatxt').modal('show');
            }else{
                mostrarAlertaError("Debe confirmar que verific&oacute; los controles de chequeo.");
            }
            return false;
        });

...
@section('page-content')
    <form class="mt-lg-3" id="frm_subcategorias" name="frm_subcategorias" autocomplete="off" method="POST" action="{{route('updateAction',$id_empresa)}}">
        @csrf
        @method('PUT')
                    ...
                    <div class="card-body p-0 text-white collapse show">
                        ...
                        <div class="p-3">
                            ...
                            @if($datos->pedir_porcentaje1==="si" && $datos->nombre =="Permiso con goce salarial")
                                <div class="mr-4"><p>¿ Permitir al usuario encargado de registrar la acción de personal, modificar el porcentaje entre un rango entre 1% y 100% ?</p></div>
                                <div class="d-flex align-items-center pb-3">
                                    ...
                                    <select class="ace-select text-secondary-d2 no-border brc-h-blue-m3" name="frm_subcategoria[{{$datos->id_subcategoria}}][editar_porcentaje1]">
                                        <option value="1" {{ $datos->editar_porcentaje1 == 1 ? 'selected' : '' }}>Si</option>
                                        <option value="0" {{ $datos->editar_porcentaje1 == 0 ? 'selected' : '' }}>No</option>
                                    </select>
                                </div>
                            @endif
                            {!! $datos->descripcion1 !!}
                            @if($datos->pedir_porcentaje1==="si" && $datos->nombre!="Permiso con goce salarial")
                                ...
                                <div class="input-group">
                                    <input type="hidden" id="frm_fecha_registro" name="frm_subcategoria[{{$datos->id_subcategoria}}][id_subcategoria]" value="{{$datos->id_subcategoria}}"/>
                                    <input type="hidden" id="frm_fecha_registro" name="frm_subcategoria[{{$datos->id_subcategoria}}][nombre]" value="{{$datos->nombre}}"/>
                                    <input type="hidden" id="frm_fecha_registro" name="frm_subcategoria[{{$datos->id_subcategoria}}][editar_porcentaje1]" value="{{$datos->editar_porcentaje1}}"/>
        ...
        <div class="mt-5 border-t-1 brc-secondary-l2 py-35 mx-n25">
            <div class="md-3 col-md-9 col-sm-12 text-nowrap">
                <a href="{{ route('administracionEmpresa') }}"  class="btn btn-outline-grey btn-text-dark btn-h-grey btn-a-grey btn-bold radius-1 d-inline-flex align-items-center pl-3px py-3px mb-1">
